...
{
    std::cout << "hello";
    std::fstream fic(fichier, std::ios::out| std::ios::trunc );
    if(fic)
    {
        fic << m_vertices.size();
        for(int i = 0; i<m_vertices.size(); i++) //Ecriture de la liste des sommmets avec leurs infos
        {
            fic << "NomEspece" << " ";
            fic << m_vertices.at(i).m_interface->m_box_label_idx.get_message() << " ";
            fic << m_vertices.at(i).get_value() << " ";
            fic << m_vertices.at(i).m_interface->m_top_box.get_posx() << " ";
            fic << m_vertices.at(i).m_interface->m_top_box.get_posy() << " ";
            fic << m_vertices.at(i).m_interface->m_img.get_pic_name()<< " ";
        }

        fic << m_edges.size();
        for(int i = 0; i<m_edges.size(); i++)
        {
            fic << m_edges.at(i).m_interface->m_label_weight.get_message() << " ";
            fic << m_edges.at(i).m_from << " ";
            fic << m_edges.at(i).m_to << " ";
        }
        fic.close();
    }
    else
...
void Graph::changer_graphe()
{
  ...
  {
      sauvegarder_graphe("graphe_test.txt");
  ...
  {
      sauvegarder_graphe("graphe_terre.txt");
    for(int i = 0; i< this->m_vertices.size(); i++)
    {
        m_interface->m_main_box.remove_child(m_vertices[i].m_interface->m_top_box);
    }
